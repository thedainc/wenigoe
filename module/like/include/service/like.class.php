 ...
 * @version 		$Id: like.class.php 5921 2013-05-14 08:40:05Z Miguel_Espinoza $
 */
class Like_Service_Like extends Phpfox_Service 
{
	...
	public function hasBeenMarked($iActionType, $sTypeId, $iItemId, $iUserId = null)
	{
		...
		$iActionId = $this->database()->select('action_id')
			...
			->where('action_type_id = ' . (int)$iActionType .' AND item_type_id = "' . $oParse->clean(str_replace('_', '-', $sTypeId)) . '" AND item_id = ' . (int)$iItemId . ' AND user_id = ' . (int)$iUserId)
			->execute('getSlaveField');
