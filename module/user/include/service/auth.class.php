 ...
 * @author  		Raymond Benc
 * @package  		Module_User
 * @version 		$Id: auth.class.php 6040 2013-06-10 18:34:06Z Raymond_Benc $
 */
		...
		{	
			if ($iUserId > 0)
			{
				...
				else
				{
					...
					{
						$this->database()->select('ls.session_hash, ls.id_hash, ls.user_id, ls.im_status, ')->leftJoin(Phpfox::getT('log_session'), 'ls', "ls.session_hash = '" . $this->database()->escape($oSession->get('session')) . "' AND ls.id_hash = '" . $this->database()->escape($oRequest->getIdHash()) . "'");					
					}
				}
