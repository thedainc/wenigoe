 ...
 * @author  		Raymond Benc
 * @package  		Module_Feed
 * @version 		$Id: feed.class.php 6140 2013-06-24 17:30:10Z Raymond_Benc $
 */
...
{	
	...
	*	but it also controls if the entire div should be shown or not, including the dislikes
	...
	public function getPhraseForLikes(&$aFeed)
	{
		...
		$iCountLikes = count($aFeed['likes']);
		
	    if (isset($aFeed['feed_is_liked']) && $aFeed['feed_is_liked'])		
	    {
			$iPhraseLimiter = (isset($aFeed['feed_total_like']) ? $aFeed['feed_total_like'] : $iCountLikes);
            if ($iPhraseLimiter == 1)
            {
	    ...
	    if (isset($aFeed['likes']) && is_array($aFeed['likes']) && $iCountLikes > 0)
	    {
            ...
            {
				if ($aLike['user_id'] == Phpfox::getUserId()) continue;
			...
			if (isset($aFeed['feed_is_liked']) && $aFeed['feed_is_liked'] && $iPhraseLimiter > 2)
			{
				
			}
			else if ( isset($aFeed['feed_total_like']) && ($aFeed['feed_total_like'] > Phpfox::getParam('feed.total_likes_to_display')))
			{
