 ...
 * @author			Raymond Benc
 * @package 		Phpfox
 * @version 		$Id: template.class.php 6117 2013-06-21 15:23:59Z Raymond_Benc $
 */
						...
						switch ($mValue)
						{
								...
								else 
								{
									...
									else
									{
										if (isset($this->_aCacheHeaders[$mKey]))
										{
                                            ...
                                            $aCacheCSS[] = str_replace(Phpfox::getParam('core.path'), '', Phpfox::getParam('core.url_file')) . 'static/' . $this->_aTheme['style_id'] . '_' . $mKey . '';
										}
										else 
										{
											/*
											...
											*/
												if ($bIsHttpsPage)
												{
                                                    ...
                                                    $aCacheCSS[] = 'static/' . $this->_aTheme['style_id'] . '_secure_' . $mKey;
												}
												else 
												{
                                                    ...
                                                    $aCacheCSS[] = 'static/' . $this->_aTheme['style_id'] . $mKey;
												}											
											// }
										}									
									}
							...
							case 'static_script':
								...
								else 
								{
									...
									{
										// $sJsCacheData .= $mKey . ',';
                                        //if (strpos($mKey, 'jquery') === false)
                                        $aCacheJs[] =  'static' . PHPFOX_DS . 'jscript' . PHPFOX_DS . $mKey;// . $sQmark . 'v=' . $iVersion;
									}
									else 
									{							
									...
									if (isset($aParts[1]) && Phpfox::isModule($aParts[1]))
									{
										...
										{			
											...
											else 
											{
                                                ...
                                                $aCacheJs[] = str_replace(Phpfox::getParam('core.path'), '', Phpfox::getParam('core.url_file')) . 'static/' . $this->_aTheme['style_id'] . '_' . $aParts[1] . '_' . $mKey;
											}
										}
									}
								}							
...
//BRODEV CODE END
	...
	public function shouldLoadDelayed($sController)
	{
		...
		// Special case
		if ($sController == 'feed.comment') return true;
		
		if (
			(in_array($sController, Phpfox::getParam('core.controllers_to_load_delayed'))) &&
