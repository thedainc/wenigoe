 ...
 * @author			Raymond Benc
 * @package 		Phpfox
 * @version 		$Id: phpfox.class.php 6160 2013-06-26 12:10:42Z Raymond_Benc $
 */
final class Phpfox
{	
	/**
	...
	{		
		...
		if (!PHPFOX_IS_AJAX_PAGE)
		{
				...
				$oTpl->setHeader(array(
							...
							'<meta http-equiv="imagetoolbar" content="no" />', // Purefan - this does not validate W3C and seems to only work in IE6, should we drop it?
							...
							if (!Phpfox::isMobile() && Phpfox::isModule('im') && Phpfox::isUser() && Phpfox::getUserBy('profile_page_id') <= 0 && Phpfox::getParam('im.enable_im_in_footer_bar') && !$oTpl->bIsSample)
							{
								...
								->setHeader(array(
										'<script type="text/javascript">oCore[\'im.is_hidden\'] = \'' . Phpfox::getUserBy('im_hide') . '\';</script>',
										 '<script type="text/javascript">$Behavior.loadMusicPlayer = function() { $Core.player.load({id: \'js_im_player\', type: \'music\'}); $Behavior.loadMusicPlayer = function() {} }</script>'
									)
								);
							}
						}
